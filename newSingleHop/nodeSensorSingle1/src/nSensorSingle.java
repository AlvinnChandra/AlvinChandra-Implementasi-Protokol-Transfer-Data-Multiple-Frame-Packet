...
public class nSensorSingle {
    ...
    // =========================================================
    // TAMBAHAN: Helper wrap-around SN (ruang 0-255, half-window 128)
    // snA lebih baru dari snB?
    // =========================================================
    private boolean isNewerSN(int snA, int snB) {
        if (snB == -1) return true;
    ...
    // =========================================================
    // FIX: processCumACK dengan deteksi wrap-around SN 0-255
    // =========================================================
    public void processCumACK(int ackedSN) {
    	synchronized (mapLock) {
    		// Gunakan isNewerSN agar handle wrap-around dengan benar
    		if (!isNewerSN(ackedSN, highestAckSN)) {
    			return;
    		}
    		...
    		for (Integer sn : unackMap.keySet()) {
    			// Hapus semua SN yang sudah di-ACK:
    			// SN dianggap "sudah di-ACK" jika bukan lebih baru dari ackedSN
    			if (!isNewerSN(sn.intValue(), ackedSN)) {
    				toRemove.add(sn);
    			}
    		}
    ...
    private void flushBuffer() {
        ...
        while (start < bufferCount) {
            ...
            synchronized (mapLock) {
                unackMap.put(new Integer(frameSN), payload);
            }
