...
//
// Peran bafe:
//   1. Terima broadcast/unicast dari BS -> relay ke affe, proses sendiri
//   2. Terima reply dari affe -> relay ke BS dengan srcAddr = affe
//      (agar BS tahu reply itu dari affe, bukan bafe)
//   3. Menu 2 (setYourTime): BS kirim unicast ke bafe DAN ke affe.
//      Karena affe tidak jangkau BS, bafe harus relay ke affe.
//      Tapi progBS kirim ke nodeAddress[2] = bafe saja untuk relay ke affe.
//      Lihat bagian WORKAROUND menu 2 di bawah.
// ============================================================
public class nSensorMulti {
    ...
    //                   BS       affe     bafe (node ini)

    int indexNode  = 2;
    int myAddress  = nodeAddress[indexNode]; // 0xBAFE

    int BS   = nodeAddress[0]; // 0xBABE
    int affe = nodeAddress[1]; // 0xAFFE  (tetangga bawah)
    // nextHop ke BS = BS (langsung terhubung)
    // prevHop ke affe = affe (langsung terhubung)
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    ...
                    // ------------------------------------------------
                    // FRAME DARI AFFE (reply atau data SENSE)
                    // Relay ke BS dengan srcAddr = affe
                    // ------------------------------------------------
                    if (srcAddr == affe) {
                        // Relay semua frame dari affe ke BS,
                        // dengan srcAddr frame = affe agar BS tahu pengirim aslinya
                        relayToBS(pesanDiterima, sequenceNumber++, affe);
                        return;
                    }
                    ...
                    // Jangan proses SENSE dari BS (tidak mungkin, tapi jaga-jaga)
                    ...
                    if (code >= 1 && code <= 5) {
                        relayToAffe(pesanDiterima, sequenceNumber++);
                    }
    ...
    // ----------------------------------------------------------------
    // PROSES COMMAND (untuk bafe sendiri)
    // Format reply sama persis dengan yang di-parse progBS di dispatch():
    //   case 1: split[1]=t1, split[2]=t2, split[3]=t3  -> RTT = t4-t1-(t3-t2)
    //   case 2: split[3]=t3 (waktu baru node)
    //   case 3: split[3]=t3 (waktu sekarang node)
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    private void relayToAffe(final String msg, final int sn) throws InterruptedException {
        boolean ok = false;
        while (!ok) {
            try {
    ...
    // ----------------------------------------------------------------
    // RELAY ke BS (forward reply/SENSE dari affe ke BS)
    // KUNCI: srcAddr diset ke pengirim ASLI (affe) bukan bafe
    // Sehingga progBS->dispatch()->indexOf(frame.getSrcAddr()) dapat affe
    // ----------------------------------------------------------------
    private void relayToBS(final String msg, final int sn, final long originalSrcAddr) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
    ...
    // ----------------------------------------------------------------
    // KIRIM KE BS (reply bafe sendiri)
    // srcAddr = myAddress (bafe)
    // ----------------------------------------------------------------
    private void sendToBS(final String msg, final int sn, final long srcAddr) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
                    try {
                        ...
                        radio.setState(AT86RF231.STATE_TX_ARET_ON);
                        fio.transmit(frame);
                        System.out.println("sent to BS: " + msg);
                        ok = true;
                    } catch (NoAckException e) {
                        // retry
                    } catch (ChannelBusyException e) {
