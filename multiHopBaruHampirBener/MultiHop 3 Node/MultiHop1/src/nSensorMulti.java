...
public class nSensorMulti {
    ...
    public void prosesHello(String[] split, long t2) throws Exception {
        ...
        sendReplyToNextHop(msg, sequenceNumber++);
    }
    ...
    // Kirim reply command (001/010/011) ke nextHop
    private void sendReplyToNextHop(final String msg, final int sn) {
        new Thread() {
            public void run() {
                boolean ok = false;
                while (!ok) {
    ...
    private void sendSenseToNextHop(final String msg, final int sn) throws InterruptedException {
        boolean ok = false;
        while (!ok) {
            try {
                Frame frame = new Frame(Frame.TYPE_DATA | Frame.ACK_REQUEST
