...
public class nSensorMulti {
    ...
    // Command 001: Hello
    // Format masuk dari BS (BABE): "001 <t1> <tsBS>"
    // Reply ke BS (BABE):          "001 <t1> <t2> <t3>"
    ...
    // Format masuk dari BS (BABE): "010 <deltaRTT/2> <t1> <tsBS>"
    // Reply ke BS (BABE):          "010 <delta_t3t2> <t3_baru>"
    ...
    // Format masuk dari BS (BABE): "011 <t1> <tsBS>"
    // Reply ke BS (BABE):          "011 <t1> <t2> <t3>"
            ...
            public void run() {
                ...
                while (!ok) {
                    try {
                        ...
                        radio.setState(AT86RF231.STATE_TX_ARET_ON);
                        fio.transmit(frame);
                        System.out.println("relay to BS" + msg);
                        ok = true;
                    } catch (RadioDriverException e) {
                        e.printStackTrace();
                    } catch (NoAckException e) {
